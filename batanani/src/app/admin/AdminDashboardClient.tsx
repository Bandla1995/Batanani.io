...
function AdminDashboardClient() {
  ...
  const { data: hairdressers = [], isLoading: hairdressersLoading } = useGetHairdressers();
   
  const { data: appointments = [], isLoading: appointmentsLoaoding } = useGetAppointments();

  // stats calculated in real time 
   
  const stats = {
    totalHairdressers: hairdressers.length,
    activeHairdressers: hairdressers.filter(hair => hair.isActive).length,
    totalAppointments: appointments.length,
    completedAppointments: appointments.filter((app) => app.status === "COMPLETED").length,
  };

  if (hairdressersLoading || appointmentsLoaoding) return < LoadingUI />;
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="mb-12 flex items-center justify-between bg-gradient-to-br from-primary/10 via-primary/5 to-background rounded-3xl p-8 border border-primary/20">
          <div className="space-y-4">
            ...
            <div>
              ...
              <p className="text-muted-foreground">
                Manage hairdressers, oversee appointments, and monitor your clients satisfaction.
              </p>
            </div>
          </div>
          ...
          {/* DASHBOARD STATS */}
          <AdminStats
            totalHairdressers={stats.totalHairdressers}
            activeHairdressers={stats.activeHairdressers}
